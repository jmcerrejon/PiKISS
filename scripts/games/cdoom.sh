...
menu() {
    ...
    do
        case $choice in 
            Doom) get_wad doom $URL_DOOM ; sudo sh -c "echo 'crispy-doom -iwad $WAD_PATH/wads/DOOM.WAD' > /usr/bin/doom" && sudo chmod +x /usr/bin/doom ;;
            Heretic) get_wad heretic $URL_HERETIC ; sudo sh -c "echo 'crispy-heretic -iwad $WAD_PATH/wads/HERETIC.WAD' > /usr/bin/heretic" && sudo chmod +x /usr/bin/heretic ;;
            Hexen) get_wad Hexen $URL_HEXEN ; sudo sh -c "echo 'crispy-hexen -iwad $WAD_PATH/wads/HEXEN.WAD' > /usr/bin/hexen" && sudo chmod +x /usr/bin/hexen ;;
            Strife) get_wad Strife $URL_STRIFE ; sudo sh -c "echo 'crispy-strife -iwad $WAD_PATH/wads/STRIFE.WAD' > /usr/bin/strife" && sudo chmod +x /usr/bin/strife ;;
        esac
    done
}
