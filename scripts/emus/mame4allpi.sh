...
mame_install(){
	...
	if [[ -e $INSTALL_DIR ]]; then
		endMessage 'MAME by Choccy already installed. Skipping...'; return 0;
	fi
...
advmame_install(){
	...
	if isPackageInstalled advancemame; then
		endMessage 'AdvancedMAME already installed. Skipping...'; return 0;
	fi
...
mame4all_install(){
    ...
    if [[ -e $INSTALL_DIR ]]; then
		endMessage 'MAME4All already installed. Skipping...'; return 0;
	fi
